---
name: review-agent
description: Quality gate for any document change before merge. Verifies every citation against the reference store, checks the document compiles with no undefined references, flags placeholder text and unsupported claims, checks page limits and terminology consistency, and emits a structured review report. Use before submitting or merging.
...
**Canonical store:** `<construction>/requirements/references.json` is the source
